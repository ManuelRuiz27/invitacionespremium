...
describe('generated API client runtime', () => {
  ...
  it('exposes seating and SVG source reads plus mutation wrappers generated from OpenAPI', async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
    ...
    await client.floorplan.svgSource('event/id', signal);
    ...
    expect(fetchImpl.mock.calls[0]?.[0]).toBe('https://api.example.com/api/v1/events/event%2Fid/floorplan/svg-source');
    ...
    expect(fetchImpl.mock.calls[1]?.[1]).toEqual(expect.objectContaining({ signal }));
    expect(fetchImpl.mock.calls.slice(2).map(([, init]) => new Headers(init?.headers).get('Idempotency-Key'))).toEqual([
      'assign-key',
      'family-key',
      'group-key',
      'update-key'

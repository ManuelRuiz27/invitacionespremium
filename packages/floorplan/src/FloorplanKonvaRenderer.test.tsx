...
describe('FloorplanKonvaRenderer de producción', () => {
  ...
  it('projects empty, partial, full, selected and read-only mapped SVG table states without a proxy fill', () => {
    ...
    const view = render(
      <FloorplanKonvaRenderer {...props({ floorplan: { ...floorplan, shapes: [mapped] }, svgSource: source })} />
    );
    ...
    view.rerender(
      <FloorplanKonvaRenderer
        {...props({
          floorplan: { ...floorplan, shapes: [{ ...mapped, occupancy: 2, availableCapacity: 2 }] },
          svgSource: source
        })}
      />
    );
    ...
    view.rerender(
      <FloorplanKonvaRenderer
        {...props({
          floorplan: { ...floorplan, shapes: [{ ...mapped, occupancy: 4, availableCapacity: 0 }] },
          svgSource: source,
          selectedId: mapped.id,
          disabled: true,
          readOnly: true
    ...
    expect(latest('Text', 'floorplan-svg-table-status').props.text).toBe(
      'Completa · 4 de 4 · Seleccionada · Solo lectura'
    );
  });

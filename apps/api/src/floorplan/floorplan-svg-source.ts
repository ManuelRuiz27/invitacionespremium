...
export function deriveFloorplanSvgSource(fileAssetId: string, bytes: Buffer): FloorplanSvgSourceDto {
  ...
  parser.on('opentag', (tag: SaxesTagNS) => {
    ...
    const node: Node = {
      ...
      attrs,
      matrix: multiply(parent?.matrix ?? identity(), transform(attrs.transform)),
      children: []
    };
  ...
  const values = (root.attrs.viewBox ?? '')
    .trim()
    .split(/[\s,]+/u)
    .map(Number);
  ...
  const visit = (node: Node): Box | undefined => {
    ...
    if (selectable.has(node.name) && own) {
      ...
      elements.push({
        sourceElementId,
        elementType: node.name as FloorplanSvgSourceDto['selectableElements'][number]['elementType'],
        bbox: normalize(own, viewBox)
      });
    }
    return own;
  };
...
function geometry(node: Node): Box | undefined {
  ...
  switch (node.name) {
    case 'rect':
      points = corners(n('x'), n('y'), n('width'), n('height'));
      break;
    case 'circle':
      points = corners(n('cx') - n('r'), n('cy') - n('r'), n('r') * 2, n('r') * 2);
      break;
    case 'ellipse':
      points = corners(n('cx') - n('rx'), n('cy') - n('ry'), n('rx') * 2, n('ry') * 2);
      break;
    case 'polygon':
    case 'polyline':
      points = pairs(node.attrs.points ?? '');
      break;
    case 'path': {
      try {
        const [minX, minY, maxX, maxY] = pathBounds(node.attrs.d ?? '');
        points = corners(minX, minY, maxX - minX, maxY - minY);
      } catch {
        return undefined;
      }
      break;
...
function corners(x: number, y: number, width: number, height: number) {
  return [
    { x, y },
    { x: x + width, y },
    { x, y: y + height },
...
function union(boxes: Box[]): Box | undefined {
  return boxes.length ? bounds(boxes.flatMap((box) => corners(box.x, box.y, box.width, box.height))) : undefined;
}
function normalize(box: Box, viewBox: FloorplanSvgSourceDto['viewBox']): Box {
  const clamp = (value: number) => Math.max(0, Math.min(1, value));
  const x = clamp((box.x - viewBox.minX) / viewBox.width);
  const y = clamp((box.y - viewBox.minY) / viewBox.height);
  return {
    x,
    y,
    width: Math.max(0, clamp((box.x + box.width - viewBox.minX) / viewBox.width) - x),
    height: Math.max(0, clamp((box.y + box.height - viewBox.minY) / viewBox.height) - y)
  };
}

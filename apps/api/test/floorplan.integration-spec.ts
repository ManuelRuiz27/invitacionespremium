...
describe('Floorplan and seating', () => {
  ...
  it('maps circular, irregular and decorative SVG elements, persists reload, and coexists with manual shapes', async () => {
    ...
    const before = await prisma.event.findUniqueOrThrow({ where: { id: fixture.event.id } });
  ...
  it('rejects nonexistent, raster, unavailable, locked, terminal and cross-scope mappings', async () => {
    const fixture = await createFixture();
    const asset = await createSvgAsset(fixture);
  ...
  it('unlinks without deleting shapes, seats or assignments and derives new SEAT table capacity', async () => {
    const fixture = await createFixture();
  ...
  it('rejects malformed canonical transforms without persisting a mapping', async () => {
    const fixture = await createFixture();
    const asset = await createSvgAsset(fixture);
    await floorplan.create(fixture.event.id, { imageAssetId: asset.id }, fixture.principal);

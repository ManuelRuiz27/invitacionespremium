...
export function mockAdminApi(user: AuthUser = platformAdmin): AdminTestApiClient {
  return {
    ...
    adminEvents: {
      ...
      activateManagedForClient: vi.fn().mockResolvedValue({ ...adminEvent, status: 'ACTIVE' }),
    ...
    adminEventPreparation: {
      ...
      uploadFloorplanSvgAsset: vi.fn(),
      floorplanAssetContent: vi.fn(),
      removeFloorplanAsset: vi.fn(),
      lockFloorplan: vi.fn(),
      setFloorplanSeatingMode: vi.fn(),
    ...
    events: { list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), activate: vi.fn(), close: vi.fn() },
